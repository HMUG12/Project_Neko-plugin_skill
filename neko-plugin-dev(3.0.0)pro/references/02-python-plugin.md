...
class MyPlugin(NekoPluginBase):
    ...
    async def on_config_change(self, old_config=None, new_config=None, **_):
        """配置变更时调用（含新旧配置；也可写成 `async def on_config_change(self, **_)` 后 `await self.config.dump()` 重新读取，更稳健）"""
        if new_config:
    ...
    # ══════════════════════════════════════════
    # AI 入口方法（不能有 keywords；上下文用 self.ctx）
    # ══════════════════════════════════════════
...
| `@plugin_entry(...)` | none（可加 `**_`） | 不能有 `keywords` | AI 入口；上下文用 `self.ctx` |
| `@ui.action(...)` | none（可加 `**_`） | 可设 `tone="primary"`, `refresh_context=True` | UI 按钮调用；上下文用 `self.ctx` |
