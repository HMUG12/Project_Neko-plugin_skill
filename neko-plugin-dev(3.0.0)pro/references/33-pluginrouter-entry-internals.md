...
async def my_entry(self,, *, param1: str):
    ...
```
...
```python
@before_entry("action")
async def before_action(self,, **kwargs):
...
```python
@after_entry("action")
async def after_action(self,, result=None, **kwargs):
...
```python
@around_entry("action")
async def around_action(self,, handler=None, **kwargs):
    """包裹 entry 执行，类似中间件"""
    start = time.time()
...
async def check_permission(self,, entry_id=None, **kwargs):
    """在所有 entry 执行前检查权限"""
...
class MusicRouter(PluginRouter):
    ...
    async def play(self,, *, song_name: str):
    ...
    async def register_genre_entries(self, genres: list[str]):
        for genre in genres:
            async def genre_handler(, genre=genre):
