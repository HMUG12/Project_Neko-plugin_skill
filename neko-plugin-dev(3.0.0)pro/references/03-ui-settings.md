...
| `Slider` | ⚠️ **官方组件库无此组件**，用 `Input`(type=number) 或 `Select` 替代 | ❌ |
...
### Slider 滑块（⚠️ 官方组件库无 `Slider`）

> 官方 `@neko/plugin-ui` **没有** `Slider` 组件。需要滑块时用 `Input`（type="number"）或 `Select` 替代：
...
- [41-ui-design-quality.md](41-ui-design-quality.md) — 反"AI 味"的 Hosted UI 设计（在受限组件集下做出干净克制的界面）