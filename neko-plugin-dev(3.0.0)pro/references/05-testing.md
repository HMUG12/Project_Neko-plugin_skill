...
class TestMyPlugin:
    ...
    async def test_async_method(self):
        p = make_plugin()
        result = await p.async_method(param="value",)
    ...
    async def test_permission_denied(self):
        ...
        p.enable_ops = True
        p.allow_read = False
        result = await p.read_file(file_path="C:\\test.txt",)
...
3. **异步方法用普通关键字参数调用** — 模拟 SDK 行为（无需 `_ctx=None`，上下文用 `self.ctx`）
